...
def get_ts_context(
    *,
    ...
    if file_io_concurrency_limit is not None:
        context.setdefault("file_io_concurrency", {})["limit"] = (
            file_io_concurrency_limit
        )
    if data_copy_concurrency_limit is not None:
        context.setdefault("data_copy_concurrency", {})["limit"] = (
...
def build_kvstore_tspec(
    ...
    *,
    ...
    directory = (
        os.path.normpath(directory).replace("gs:/", "gs://").replace("yt:/", "yt://")
    )
...
def get_sharding_tensorstore_spec(directory: str, param_name: str) -> dict[str, Any]:
    kvstore_tspec = build_kvstore_tspec(directory, name="_sharding", use_ocdbt=False)
